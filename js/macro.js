...
function main() {
  ...
  $slider.slider({
	...
	// run changeUOM() once on page load
  changeUOM(); 
	// change units when a tab is clicked
  ...
  $calcButton.click(calculate); // run calculate when clicking the Calculate button
  ...
  function changeUOM() {
    if (imperial) {
      ...
      $heightInputIn.closest("div").show();
			$heightInputIn.val("");
      $slider.slider({
        min: 0,
        max: 2.5,
      ...
      $heightInputIn.closest("div").hide();
			$heightInputIn.val("");
      $slider.slider({
        min: 0,
        max: 1.15,
